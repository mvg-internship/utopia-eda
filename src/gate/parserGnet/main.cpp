...
GateSymbol functionGateSymbol(const size_t type, std::map<int,std::string> &typeRTLIL, const int a){
  GateSymbol func;
  func=GateSymbol::XXX;
...
void printMemory(const Yosys::hashlib::dict<Yosys::RTLIL::IdString, Yosys::RTLIL::Memory*> &memories, std::ostream &out){
...
void printActions(const std::vector< Yosys::RTLIL::SigSig > &actions, std::ostream &out){
  for (auto &it : actions){
    out << "    " << (*it.first.as_chunk().wire).name.index_ << "\n";
  }
}
void printSyncs(const std::vector<Yosys::RTLIL::SyncRule *> &syncs, std::ostream &out){
  out << "  syncs\n";
...
void printProcesses(const Yosys::hashlib::dict<Yosys::RTLIL::IdString, Yosys::RTLIL::Process*> &processes, std::ostream &out){
...
void printPorts(const std::vector<Yosys::RTLIL::IdString> &ports, std::ostream &out){
  out << "Ports count: " << ports.size() << "\n";
  for (auto &it1 : ports){
    out << "  port " << it1.index_ << "\n";
  }
}
void printParams(const std::pair<Yosys::RTLIL::IdString, Yosys::RTLIL::Module*> &m, std::ostream &out){
...
void printParsed(const Yosys::RTLIL::Design &des, std::ostream &out){
